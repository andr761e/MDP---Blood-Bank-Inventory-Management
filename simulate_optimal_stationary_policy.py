...
INVENTORY_CAP = 8
MAX_ORDER = 5
PRODUCTION_DAYS = {0, 1, 2, 3, 4}   # Monday-Friday only
...
# Simulation controls
START_STATE = (0, 0, 0, 0, 0)   # (day, x1, x2, x3, x4)
SIMULATION_DAYS = 140
RANDOM_SEED = 42
N_REPLICATIONS = 1              # increase if you want repeated runs

# Output files
POLICY_OUTPUT_XLSX_PATH = Path("data/optimal_stationary_policy_for_simulation.xlsx")
SIM_OUTPUT_XLSX_PATH = Path("data/stationary_policy_simulation.xlsx")
...
def step_dynamics_detailed(
    ...
    State at start of day:
        (day, x1, ..., x_{m-1})
    New order enters with m days left.

    Returns detailed one-day evolution.
    """
    day = state[0]
    inv = list(state[1:])
...
# ============================================================
# COMPUTE OPTIMAL STATIONARY POLICY (same logic as LP script)
# ============================================================
...
# ============================================================
# SIMULATION UNDER THE OPTIMAL POLICY
# ============================================================
...
def simulate_one_path(
    ...
    for t in range(n_days):
        ...
        row = {
            ...
            "fifo_used": str(details["fifo_used"]),
        ...
        # Also store x1,...,x_{m-1} for readability
...
def build_replication_summary(replication_results: List[pd.DataFrame]) -> pd.DataFrame:
    ...
    for i, sim_df in enumerate(replication_results, start=1):
        rows.append({
            ...
            "avg_cost_per_week": sim_df.groupby(sim_df["t"] // 7)["period_cost"].sum().mean(),
