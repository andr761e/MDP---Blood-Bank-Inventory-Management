...
def solve_stationary_average_cost_lp_for_terminal_values(
    ...
    print_stage_header("[Stationary terminal model] Solving average-cost LP")

    actions_by_state = {
        s: feasible_actions_regular(
            s,
            INVENTORY_CAP,
            MAX_ORDER,
            REGULAR_PRODUCTION_DAYS,
        )
        for s in states
    }
    ...
    print(f"Stationary states:             {len(states):,}", flush=True)
    print(f"Stationary state-action pairs: {total_state_action_pairs:,}", flush=True)

    transitions: Dict[tuple[State, int], Dict[State, float]] = {}
    expected_cost: Dict[tuple[State, int], float] = {}

    # ------------------------------------------------------------
    # 1. Build transition distributions and expected costs
    # ------------------------------------------------------------
    print("\n[1/4] Building stationary transition distributions...", flush=True)

    progress = make_progress_printer(
        total=total_state_action_pairs,
        label="[1/4] Stationary transition building",
    )
    ...
    print(
        f"[1/4] Transition building complete. "
        f"Elapsed: {time.perf_counter() - transition_start:.1f}s",
        flush=True,
    )
    ...
    # ------------------------------------------------------------
    print("\n[2/4] Building stationary Gurobi LP model...", flush=True)

    model = gp.Model("stationary_average_cost_dual_lp_for_terminal_values")

    # Set to 1 while debugging large parameter choices.
    # This gives Gurobi's own optimizer log.
    ...
    progress = make_progress_printer(
        total=total_state_action_pairs,
        label="[2/4] Stationary constraint building",
    )
    ...
    print(
        f"[2/4] Constraint building complete. "
        f"Elapsed: {time.perf_counter() - constraint_start:.1f}s",
        flush=True,
    )

    print(
        f"LP size: {len(states):,} h-variables + 1 g-variable, "
        f"{total_state_action_pairs:,} Bellman constraints.",
        flush=True,
    )
    ...
    # 3. Optimize LP
    # ------------------------------------------------------------
    print("\n[3/4] Optimizing stationary LP with Gurobi...", flush=True)
    print(
        "Note: Gurobi cannot give exact 10%, 20%, ... progress, "
        "because the number of optimizer iterations is not known beforehand.",
        flush=True,
    )
    ...
    if model.Status != GRB.OPTIMAL:
        raise RuntimeError(
            f"Gurobi did not find an optimal stationary solution. "
    ...
    print(
        f"[3/4] Stationary LP solved. "
        f"g* per day = {g_star:.6f}. "
        f"Elapsed: {optimize_elapsed:.1f}s",
        flush=True,
    )

    # ------------------------------------------------------------
    # 4. Extract deterministic stationary policy
    # ------------------------------------------------------------
    print("\n[4/4] Extracting deterministic stationary policy...", flush=True)

    det_policy: Dict[State, int] = {}

    progress = make_progress_printer(
        total=len(states),
        label="[4/4] Stationary policy extraction",
    )
    ...
    for state_counter, s in enumerate(states, start=1):
        ...
        # If several actions are numerically tied, choose the smallest order.
        det_policy[s] = min(best_actions)

        progress(state_counter)

    print(
        f"[4/4] Stationary policy extraction complete. "
        f"Elapsed: {time.perf_counter() - policy_start:.1f}s",
        flush=True,
    )

    print(
        f"\nStationary terminal model complete. "
        f"g* per day = {g_star:.6f}, "
        f"g* per week = {7.0 * g_star:.6f}",
        flush=True,
    )
...
def make_progress_printer(total: int, label: str, step_percent: int = 10):
    """
    Print progress at 10%, 20%, ..., 100% for loops with known total work.
    """
...
def solve_finite_horizon_backward_induction(
    ...
    print_stage_header("[Finite horizon] Solving non-stationary model by backward induction")
    ...
    if missing_terminal_states:
        raise ValueError(
            "Some terminal states are not present in the stationary state space, "
            "so terminal relative values are missing.\n"
            f"First few missing states: {missing_terminal_states[:10]}"
        )

    total_states_to_solve = sum(len(reachable_by_t[t]) for t in range(HORIZON_DAYS))

    print(f"Finite-horizon state values to compute: {total_states_to_solve:,}", flush=True)

    progress = make_progress_printer(
        total=total_states_to_solve,
    ...
    for t in reversed(range(HORIZON_DAYS)):
        ...
        states_this_stage = reachable_by_t[t]

        stage_action_count = 0

        print(
            f"  Solving t={t:2d} ({row['date']}, {row['weekday']}): "
            f"{len(states_this_stage):,} states, "
            f"can_produce={bool(row['can_produce'])}",
            flush=True,
        )

        for s in states_this_stage:
            scores: Dict[int, float] = {}
            expected_immediate_costs: Dict[int, float] = {}

            actions = feasible_actions_at_time(
                s,
                t,
                calendar_df,
                INVENTORY_CAP,
            ...
            stage_action_count += len(actions)
        ...
        print(
            f"  Finished t={t:2d}: "
            f"{len(states_this_stage):,} states, "
            f"{stage_action_count:,} state-action evaluations, "
    ...
    policy_df = pd.DataFrame(rows).sort_values(
        ["t", "weekday", "total_stock", "optimal_order"]
    )

    print("Backward induction complete.", flush=True)
